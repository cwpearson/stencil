...
TEMPLATE_TEST_CASE("MTRegionSender MTRegionRecver", "[cuda][mpi][template]", int,
                   double) {
  ...
  SECTION("+x +y +z corner") {
    ...
    MTRegionSender<AnySender> sender(d0, srcRank, srcGPU, dstRank, dstGPU, dir);
    MTRegionRecver<AnyRecver> recver(d1, srcRank, srcGPU, dstRank, dstGPU, dir);
